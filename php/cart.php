 ...
 <html>  
      ...
      <body>  
           ...
           <div class="container" style="width:800px;">  
                ...
                {  
                     $insert_order = "  
                     INSERT INTO tbl_order(id, creation_date, order_status)  
                ...
                {  
                     ...
                     '; 
                     $det = ' SELECT * FROM farmerdetails WHERE username= "'.$_SESSION["username"].'" '; 

                     $result = mysqli_query($conn, $query); 
                     $sql = mysqli_query($conn,$det); 

                     while($row = mysqli_fetch_array($sql)) 
                     {
                      $customer_details = '  
                          <label>'.ucfirst($row["username"]).'</label>  
                          <p>'.$row["location"].'</p>  
                          <p>'.$row["email"].'</p>  
                          <p>'.$row["phone"].'</p>  
                          '; 
